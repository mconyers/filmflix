...
class AddRecord(FlaskForm):
    ...
    name = StringField('Sock name', [InputRequired(),
                                     Regexp(r'^[A-Za-z\s\-\']+$',
                                            message="Invalid sock name"),
                                     Length(min=3, max=25,
                                            message="Invalid sock name length")
                                     ])
...
def select_record(letters):
    ...
    a, b = list(letters)
    socks = Sock.query.filter(Sock.name.between(a, b)
                              ).order_by(Sock.name).all()
